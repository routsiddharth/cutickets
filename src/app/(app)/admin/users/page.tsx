...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  ...
  const users = await prisma.user.findMany({
    ...
    take: PAGE_SIZE,
    select: {
      id: true, name: true, email: true, image: true, school: true, classYear: true, createdAt: true, bannedAt: true,
  ...
  return (
    <main className="max-w-5xl mx-auto px-5 sm:px-7 py-8">
      ...
      <p className="text-sm text-muted mt-1 mb-6">{query ? `${total} matching` : `${total} regular user${total === 1 ? "" : "s"}`}</p>
      ...
      {users.length === 0 ? (
        <div className="border-y border-line py-8 text-sm text-muted">{query ? "No users match that search." : "No regular users have signed up yet."}</div>
      ) : (
