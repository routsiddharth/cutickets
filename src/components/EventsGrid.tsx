...
function EventCard({
  ...
  return (
    // Transform lives on this wrapper, never on the element that also carries
    // overflow-hidden below — Safari fails to clip an overflow-hidden box
    // that has its own transform (even a resting translateY(0) still counts),
    // which let the card's rounded corners and the tear notch's clip both
    // render un-clipped. Keeping them on separate elements sidesteps that.
    <div
      style={{
    ...
    >
      <Link
        ...
        className="group block h-full bg-card rounded-2xl overflow-hidden"
        style={{
          boxShadow: hovered
            ? "inset 0 0 0 1px #E0D8C6, 0 20px 36px rgba(22,35,58,.16)"
            : "inset 0 0 0 1px #EDE7DA, 0 1px 2px rgba(22,35,58,.04)",
          // The tear notch on the left/right edges is a flat cutout color,
          // not a real hole, so a shadow reaching the card's own edges shows
          // up as a mismatched pale disc instead of blending into the page.
          // Clipping the shadow to the card's sides/top and letting it bleed
          // only below keeps it off the notches (and reads as a more natural
      ...
      >
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between gap-3 pt-1">
            ...
            >
              <span
                className="relative overflow-hidden inline-block text-xs font-medium px-3.5 py-2 rounded-lg"
                style={
                  hasAvailable
                    ? {
                        color: "#fff",
  ...
  return (
    ...
    >
      <Link
        ...
        onMouseEnter={onHoverStart}
        onMouseLeave={onHoverEnd}
        className="group block h-full rounded-2xl overflow-hidden bg-card"
        style={{
          boxShadow: hovered
